...
public class CustomHandBehaviour : MonoBehaviour
{
    ...
    private void UpdateTeleportStatus(Vector3 Origin, Vector3 Target, XRRaycasterUtils.TeleportState teleportState)
    {
        ...
        if (HandRaycaster.HitResult.gameObject && HandRaycaster.HitResult.gameObject.GetComponent<TeleportPoint>())
        {
            var teleportPoint = HandRaycaster.HitResult.gameObject.GetComponent<TeleportPoint>();
            teleportPoint.BeingSelect(HandRaycaster);
            targetTemp = teleportPoint.transform.position;
            selectingPoint = teleportPoint;
        }

        Vector3 direction = targetTemp - XRManager.Instance.transform.position;
